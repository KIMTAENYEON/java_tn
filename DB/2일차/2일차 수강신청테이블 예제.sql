-- 1. university_tn DB 생성, 있으면 삭제 후 생성
drop schema if exists university_tn;
CREATE SCHEMA university_tn ;
-- 2. university_tn DB 선택
use university_tn ;
-- 3. student 테이블 생성
-- 비밀번호를 255자로 하는 이유 : 일반적으로 비밀번호는 노출되면 안되기 때문에 암호화가 이루어짐
-- 암호화가 이루어지면 짧은 비밀번호가 긴 문자열로 바뀜
drop table if exists university_tn. student;
CREATE TABLE `university_tn`.`student` (
...
-- 테이블명 앞에 DB명이 안 붙으면 현재 선택된 DB에 테이블을 활용할 수 없다
-- world DB가 선택된 상태에서 create table student();를 하면 world DB안에 student 테이블을 생성
-- world DB가 선택된 상태에서 create table university_tn. student();를 하면 
-- 선택된 DB와 상관없이 university_tn DB안에 student 테이블을 생성

-- 4. professor 테이블 생성
drop table if exists `university_tn`.`professor`;
CREATE TABLE `university_tn`.`professor` (
  ...
  -- 5. subject 테이블 생성
  drop table if exists `university_tn`.`subject`;
  CREATE TABLE `university_tn`.`subject` (
...
-- 6. course 테이블 생성
drop table if exists `university_tn`.`course`;
CREATE TABLE `university_tn`.`course` (
  ...
  CONSTRAINT `co_st_num`
    ...
    REFERENCES `university_tn`.`student` (`st_num`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `co_su_num`
    FOREIGN KEY (`co_su_num`)
    REFERENCES `university_tn`.`subject` (`su_num`) ON DELETE CASCADE ON UPDATE CASCADE);
    
    -- 7. lecture 테이블 생성
    drop table if exists `university_tn`.`lecture`;
    CREATE TABLE `university_tn`.`lecture` (
  ...
  CONSTRAINT `le_pr_num`
    ...
    REFERENCES `university_tn`.`professor` (`pr_num`) ON DELETE CASCADE ON UPDATE CASCADE,
